...
type RightMaintenanceConditions struct {
}

type RightMaintenanceConditionsArgs struct {
	QtCustomers int `json:"qtCustomers"`

	SchedulingTime int `json:"schedulingTime"`
}
...
type Asset struct {
	...
	RightMaintenanceConditions RightMaintenanceConditions
}
...
func (s *SmartContract) ClauseRightMaintenanceConditions(ctx contractapi.TransactionContextInterface, assetId string, args RightMaintenanceConditionsArgs) (string, bool, error) {
	...
	isValid = isValid && args.QtCustomers < 500

	isValid = isValid && args.SchedulingTime >= 48

	if !isValid {
		return executionId, isValid, fmt.Errorf("Unauthorised maintenance")
	}
